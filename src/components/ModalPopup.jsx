...
export function ModalPopup({ onOpen, onClose, onCreate, taskToEdit, onEdit}) {
    ...
    const {id, register, handleSubmit, formState: { errors }, reset } = useForm({
    ...
    return (
        <>
            <Modal show={onOpen} onClose={() => onClose()}>
                <Modal.Header>{taskToEdit ? 'Edi Task' : 'Add Task'}</Modal.Header>
                <Modal.Body>
                    <div className="space-y-6">
                        <form action="" className="flex flex-wrap" onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <div className="w-1/3 mt-2">
                                ...
                                <Select defaultValue={formData.priority} id="assign" {...register('priority')}>
